...
parfor i = 1:length(allfclock)
    ...
    if fix(curfcomp/compprec) == curfcomp/compprec
        ...
        % XXX bitmultiplication is missing in next equation! XXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXX
        samples = periodsperamp.*adcfs./(curfsig*bitmult);
...
if isempty(id)
    error('no result found! increase search range...')
endif
id = id(1);
